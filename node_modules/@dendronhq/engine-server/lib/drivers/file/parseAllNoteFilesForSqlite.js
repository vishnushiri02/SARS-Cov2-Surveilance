...
exports.parseAllNoteFilesForSqlite = void 0;
...
const SqliteQueryUtils_1 = require("../sqlite/SqliteQueryUtils");
const LinksTableUtils_1 = require("../sqlite/tables/LinksTableUtils");
const NotePropsTableUtils_1 = require("../sqlite/tables/NotePropsTableUtils");
/**
 ...
 * @returns
 */
async function parseAllNoteFilesForSqlite(files, vault, db, root, enableLinkCandidates = false, logger) {
    var _a, _b, _c;
    const ctx = "parseAllNoteFilesForSqlite";
    const start = process.hrtime();
    const vaultName = (_a = vault.name) !== null && _a !== void 0 ? _a : vault.fsPath;
    ...
    const vaultIdResp = await sqlite_1.VaultsTableUtils.getIdByFsPath(db, vault.fsPath);
    let vaultId;
    if (vaultIdResp.isOk()) {
        ...
        return (0, common_all_1.err)(vaultIdResp.error);
    }
    ...
    const fileStateData = files.map((file) => {
        ...
        const content = fs_extra_1.default.readFileSync(path_1.default.join(root, file), {
            encoding: "utf8",
        });
        const { name } = path_1.default.parse(file);
        return { fname: (0, common_all_1.cleanName)(name), contentHash: (0, common_all_1.genHash)(content), content };
    });
    logger === null || logger === void 0 ? void 0 : logger.info({
        ctx,
        msg: `${files.length} files read from file system`,
        duration: (0, common_server_1.getDurationMilliseconds)(filesStateDataStart),
        vault: vaultName,
    });
    ...
    if (addedFileResp.isErr()) {
        return (0, common_all_1.err)(addedFileResp.error);
    }
    // Calculate content hashes for all newly added files
    const addedNotes = addedFileResp.value.map((fname) => {
        const content = contentDictionary[fname];
        const props = (0, common_all_1.string2Note)({ content, fname: (0, common_all_1.cleanName)(fname), vault });
        props.contentHash = (0, common_all_1.genHash)(content);
        return props;
    });
    logger === null || logger === void 0 ? void 0 : logger.info({
        ctx,
        msg: `Finished calculating added files. ${addedFileResp.value.length} files have been added.`,
        duration: (0, common_server_1.getDurationMilliseconds)(addedFileStart),
        vault: vaultName,
    });
    ...
    const getUpdatedFilesResp = await getUpdatedFiles(db, fileStateData, vaultId, logger);
    if (getUpdatedFilesResp.isErr()) {
        return (0, common_all_1.err)(getUpdatedFilesResp.error);
    }
    const updatedNotes = getUpdatedFilesResp.value.map((fname) => {
        const content = contentDictionary[fname];
        const props = (0, common_all_1.string2Note)({ content, fname: (0, common_all_1.cleanName)(fname), vault });
        props.contentHash = (0, common_all_1.genHash)(content);
        return props;
    ...
    logger === null || logger === void 0 ? void 0 : logger.info({
        ...
        duration: (0, common_server_1.getDurationMilliseconds)(getUpdatedFilesStart),
        vault: vaultName,
    });
    if (updatedNotes.length > 0) {
        // We need to delete all links from updated notes, since they will get re-processed
        const linkDeleteStart = process.hrtime();
        const linkDeleteRes = await deleteLinksForUpdatedNotes(db, updatedNotes.map((props) => props.fname), vaultId, logger);
        if (linkDeleteRes.isErr()) {
            return (0, common_all_1.err)(linkDeleteRes.error);
        }
        logger === null || logger === void 0 ? void 0 : logger.info({
            ctx,
            msg: `Finished deleting links for ${updatedNotes.length} updated notes`,
            duration: (0, common_server_1.getDurationMilliseconds)(linkDeleteStart),
            vault: vaultName,
        });
    }
    // For all deleted notes, remove them from the DB
    const deleteRemovedFilesFromDBStart = process.hrtime();
    const deleteRes = await deleteRemovedFilesFromDB(db, Object.keys(contentDictionary), vaultId, logger);
    if (deleteRes.isErr()) {
        return (0, common_all_1.err)(deleteRes.error);
    }
    logger === null || logger === void 0 ? void 0 : logger.info({
        ctx,
        msg: `Finished deleting removed files from DB`,
        duration: (0, common_server_1.getDurationMilliseconds)(deleteRemovedFilesFromDBStart),
        vault: vaultName,
    });
    ...
    await Promise.all(allNotesToProcess.map((note) => {
    ...
    logger === null || logger === void 0 ? void 0 : logger.info({
        ...
        duration: (0, common_server_1.getDurationMilliseconds)(processNotePropsStart),
        vault: vaultName,
    });
    ...
    if (updatedNotes.length > 0) {
        ...
        const updateLinksForChangedNoteIdResult = await updateLinksForChangedNoteId(db, updatedNotes, vaultId);
        if (updateLinksForChangedNoteIdResult.isErr()) {
            return (0, common_all_1.err)(updateLinksForChangedNoteIdResult.error);
        }
        logger === null || logger === void 0 ? void 0 : logger.info({
            ctx,
            msg: `updateLinksForChangedNoteId ${updatedNotes.length} updated notes.`,
            duration: (0, common_server_1.getDurationMilliseconds)(updateLinksForChangedNoteIdStart),
            vault: vaultName,
        });
        ...
        const purgeChangedIdsResult = await purgeDBForUpdatedNotesWithChangedNoteId(db, updatedNotes, vaultId);
        if (purgeChangedIdsResult.isErr()) {
            return (0, common_all_1.err)(purgeChangedIdsResult.error);
        }
        logger === null || logger === void 0 ? void 0 : logger.info({
            ctx,
            msg: `purgeDBForUpdatedNotesWithChangedNoteId ran on ${updatedNotes.length} updated notes.`,
            duration: (0, common_server_1.getDurationMilliseconds)(purgeChangedIdsResultStart),
            vault: vaultName,
        });
    }
    ...
    if (addedNotes.length > 0) {
        ...
        const updateUnresolvedLinksForAddedNotesResult = await LinksTableUtils_1.LinksTableUtils.updateUnresolvedLinksForAddedNotes(db, addedNotes, (_b = vault.name) !== null && _b !== void 0 ? _b : vault.fsPath);
        logger === null || logger === void 0 ? void 0 : logger.info({
            ctx,
            msg: `updateUnresolvedLinksForAddedNotes ran on ${addedNotes.length} added notes`,
            duration: (0, common_server_1.getDurationMilliseconds)(updateUnresolvedLinksStart),
            vault: vaultName,
        });
        if (updateUnresolvedLinksForAddedNotesResult.isErr()) {
            return (0, common_all_1.err)(updateUnresolvedLinksForAddedNotesResult.error);
        }
        const insertLinksThatBecameAmbiguousResultStart = process.hrtime();
        const insertLinksThatBecameAmbiguousResult = await LinksTableUtils_1.LinksTableUtils.InsertLinksThatBecameAmbiguous(db, addedNotes.map((props) => {
            return { fname: props.fname, id: props.id };
        }));
        logger === null || logger === void 0 ? void 0 : logger.info({
            ctx,
            msg: `insertLinksThatBecameAmbiguous ran on ${addedNotes.length} added notes`,
            duration: (0, common_server_1.getDurationMilliseconds)(insertLinksThatBecameAmbiguousResultStart),
            vault: vaultName,
        });
        if (insertLinksThatBecameAmbiguousResult.isErr()) {
            return (0, common_all_1.err)(insertLinksThatBecameAmbiguousResult.error);
        }
        ...
        const addHierarchyStubsForAddedNotesResult = await addAncestorStubs(db, addedNotes, vaultId, logger);
        logger === null || logger === void 0 ? void 0 : logger.info({
            ctx,
            msg: `addHierarchyStubsForAddedNotes ran on ${addedNotes.length} added notes`,
            duration: (0, common_server_1.getDurationMilliseconds)(addHierarchyStubsForAddedNotesStart),
            vault: vaultName,
        });
        if (addHierarchyStubsForAddedNotesResult.isErr()) {
            return (0, common_all_1.err)(addHierarchyStubsForAddedNotesResult.error);
        }
        else {
        ...
        const processReplacedStubsResult = await processReplacedStubs(db, addedNotes, vaultId, logger);
        logger === null || logger === void 0 ? void 0 : logger.info({
            ctx,
            msg: `processReplacedStubs ran on ${addedNotes.length} added notes`,
            duration: (0, common_server_1.getDurationMilliseconds)(processReplacedStubsStart),
            vault: vaultName,
        });
        if (processReplacedStubsResult.isErr()) {
            return (0, common_all_1.err)(processReplacedStubsResult.error);
        }
    }
    // For all added/updated notes, add their child->parent linkages now
    const bulkProcessParentLinksStart = process.hrtime();
    const bulkProcessParentLinksResult = await processParentLinks(db, allNotesToProcess, vaultId);
    logger === null || logger === void 0 ? void 0 : logger.info({
        ctx,
        msg: `bulkProcessParentLinks ran on ${allNotesToProcess.length} notes`,
        duration: (0, common_server_1.getDurationMilliseconds)(bulkProcessParentLinksStart),
        vault: vaultName,
    });
    if (bulkProcessParentLinksResult.isErr()) {
        return (0, common_all_1.err)(bulkProcessParentLinksResult.error);
    }
    // For all added/updated notes, process their links (wikilinks, refs, etc.)
    const processForwardLinksStart = process.hrtime();
    await processForwardLinks(db, allNotesToProcess, {});
    logger === null || logger === void 0 ? void 0 : logger.info({
        ctx,
        msg: `processForwardLinks ran on ${allNotesToProcess.length} notes`,
        duration: (0, common_server_1.getDurationMilliseconds)(processForwardLinksStart),
        vault: vaultName,
    });
    if (enableLinkCandidates) {
        const bulkProcessLinkCandidatesStart = process.hrtime();
        await bulkProcessLinkCandidates(db, allNotesToProcess, {});
        logger === null || logger === void 0 ? void 0 : logger.info({
            ctx,
            msg: `bulkProcessLinkCandidates ran on ${allNotesToProcess.length} notes`,
            duration: (0, common_server_1.getDurationMilliseconds)(bulkProcessLinkCandidatesStart),
            vault: vaultName,
        });
    }
    logger === null || logger === void 0 ? void 0 : logger.info({
        ctx,
        msg: `end processing for vault ${(_c = vault.name) !== null && _c !== void 0 ? _c : vault.fsPath}`,
        duration: (0, common_server_1.getDurationMilliseconds)(start),
        vault: vaultName,
    });
    return (0, common_all_1.ok)(null);
}
exports.parseAllNoteFilesForSqlite = parseAllNoteFilesForSqlite;
// Helper Functions
async function addVaultToDb(vault, db) {
    var _a;
    return sqlite_1.VaultsTableUtils.insert(db, {
        name: (_a = vault.name) !== null && _a !== void 0 ? _a : vault.fsPath,
        fsPath: vault.fsPath,
    });
}
function getAddedFiles(db, allFiles, vaultId, logger) {
    if (allFiles.length === 0) {
        return (0, common_all_1.okAsync)([]);
    }
    const values = allFiles
        .map((fsPath) => `('${(0, common_all_1.cleanName)(path_1.default.parse(fsPath).name)}')`)
        .join(",");
    const sql = [
        `WITH T(fname) AS`,
        `(VALUES ${values})`,
    ...
    return SqliteQueryUtils_1.SqliteQueryUtils.all(db, sql, logger).map((rows) => rows.map((row) => row.fname));
}
function getUpdatedFiles(db, allFiles, vaultId, logger) {
    logger === null || logger === void 0 ? void 0 : logger.info({
    ...
    return SqliteQueryUtils_1.SqliteQueryUtils.all(db, sql, logger).andThen((updatedRows) => {
        if (updatedRows.length === 0) {
            return common_all_1.ResultAsync.fromPromise(Promise.resolve([]), (e) => {
                return e;
            });
        }
        ...
        return SqliteQueryUtils_1.SqliteQueryUtils.all(db, sql2, logger).map((rows) => rows.map((row) => row.fname));
    });
}
async function processNoteProps(note, db) {
    await NotePropsTableUtils_1.NotePropsTableUtils.insert(db, note);
    const vaultIdResp = await sqlite_1.VaultsTableUtils.getIdByFsPath(db, note.vault.fsPath);
    if (vaultIdResp.isErr()) {
        return;
    }
    await sqlite_1.VaultNotesTableUtils.insert(db, new sqlite_1.VaultNotesTableRow(vaultIdResp.value, note.id));
...
function processParentLinks(db, notes, vaultId) {
    ...
    return sqlite_1.HierarchyTableUtils.bulkInsertWithParentAsFname(db, lodash_1.default.compact(data));
}
async function processForwardLinks(db, notes, config) {
    return Promise.all(notes
        .filter((note) => !note.stub)
        .map((note) => {
        const links = unified_1.LinkUtils.findLinksFromBody({ note, config });
        const data = links
        ...
        return LinksTableUtils_1.LinksTableUtils.bulkInsertLinkWithSinkAsFname(db, lodash_1.default.compact(data));
    }));
}
async function bulkProcessLinkCandidates(db, notes, config) {
    // This method does one INSERT per note:
    return Promise.all(notes.map((note) => {
        const content = note.body;
        const remark = unified_1.MDUtilsV5.procRemarkParse({ mode: unified_1.ProcMode.FULL }, {
            noteToRender: note,
            fname: note.fname,
            vault: note.vault,
        ...
        const textNodes = [];
        (0, unified_1.visit)(ast, [unified_1.DendronASTTypes.TEXT], (node, _index, parent) => {
        ...
        return LinksTableUtils_1.LinksTableUtils.bulkInsertLinkCandidatesWithSinkAsFname(db, data);
    }));
}
/**
 ...
 * @returns
 */
function processReplacedStubs(db, addedNotes, vaultId, logger) {
    if (addedNotes.length === 0) {
        return (0, common_all_1.okAsync)(null);
    }
    ...
    return SqliteQueryUtils_1.SqliteQueryUtils.all(db, sql1, logger).andThen((replacedStubs) => {
      ...
      WHERE T.oldId = Hierarchy.parentId`;
        return SqliteQueryUtils_1.SqliteQueryUtils.run(db, sql2, logger).andThen(() => {
        ...
        (
          ...
          WHERE T.oldId = NoteProps.id
            ...
            return SqliteQueryUtils_1.SqliteQueryUtils.run(db, sql3, logger);
        });
    });
}
/**
 ...
 */
function addAncestorStubs(db, addedNotes, vaultId, logger) {
    let potentialStubNames = [];
    ...
    potentialStubNames = lodash_1.default.uniq(potentialStubNames);
    ...
    WHERE vaultId = ${vaultId}`;
    return SqliteQueryUtils_1.SqliteQueryUtils.all(db, sql1, logger)
        ...
        const stubProps = stubsToAdd.map((fname) => {
            return { fname, id: (0, common_all_1.genUUID)() };
        });
        const stubInsertionValues = stubProps
            .map((d) => `('${d.id}', '${d.fname}', '${common_all_1.NoteUtils.genTitle(d.fname)}')`)
            .join(",");
        ...
        return SqliteQueryUtils_1.SqliteQueryUtils.run(db, sql2, logger).map(() => stubProps);
    })
        .andThen((stubProps) => {
        ...
        return sqlite_1.VaultNotesTableUtils.bulkInsert(db, stubProps.map((prop) => {
            ...
            return stubProps.map((fname) => {
                return {
                    ...
                    title: "",
                    desc: "",
                    created: common_all_1.Time.now().toMillis(),
                    updated: common_all_1.Time.now().toMillis(),
                    stub: true,
                };
            });
        });
 ...
 * @returns
 */
function purgeDBForUpdatedNotesWithChangedNoteId(db, updatedNotes, vaultId, logger) {
    ...
    return SqliteQueryUtils_1.SqliteQueryUtils.run(db, sql, logger);
}
/**
 ...
 * @returns
 */
function updateLinksForChangedNoteId(db, updatedNotes, vaultId, logger) {
    const values = updatedNotes
        .map((props) => `('${props.fname}','${props.id}')`)
        .join(",");
    const sql = `
...
function deleteRemovedFilesFromDB(db, remainingFiles, vaultId, logger) {
    let sql;
...
function deleteLinksForUpdatedNotes(db, updatedNoteFnames, vaultId, logger) {
...
//# sourceMappingURL=parseAllNoteFilesForSqlite.js.map